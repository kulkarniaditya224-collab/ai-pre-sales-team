...
# ---------------------------------------------------------------------------
# ENV VAR PRIORITY: VPS env vars > Supabase settings table > hardcoded defaults.
...
async def _adb():
    try:
        ...
        return await acreate_client(_default("SUPABASE_URL"), _default("SUPABASE_SERVICE_KEY"))
    except ImportError:
...
async def get_all_settings() -> dict:
    ...
    # VPS env vars win over Supabase values
    out: dict = {}
    for k in KNOWN_KEYS:
        env_val = os.getenv(k, "")
        effective = env_val if env_val else db_vals.get(k, "")
        source = "env" if env_val else ("db" if db_vals.get(k) else "none")
        if k in SENSITIVE_KEYS:
            out[k] = {"value": "", "configured": bool(effective), "source": source}
        else:
            out[k] = {"value": effective, "configured": bool(effective), "source": source}
    return out
...
async def get_setting(key: str, default: str = "") -> str:
    """VPS env var wins. Supabase is fallback only."""
