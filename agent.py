...
def _build_session(tools: list, system_prompt: str) -> AgentSession:
    ...
    tts = None  # No separate TTS needed for Gemini Live; pipeline uses LLM streaming
    return AgentSession(
        vad=silero.VAD.load(),
        stt=stt,
        llm=llm_inst,
...
class OutboundAssistant(Agent):
    """Outbound voice AI agent — passes tools=[] to avoid duplicate tool error."""
...
async def entrypoint(ctx: agents.JobContext):
    ...
    5. Keep session alive via participant_disconnected event.
    """
    ...
    # Initialize tool context (reuses transfer pattern from LIvekitAIVoice)
